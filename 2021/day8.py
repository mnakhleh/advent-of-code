...
TEST2 = """be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |
...
INPUT = """bfdcga cgdfe egfda cbfeda ec efc gcfbd ebcg gdfbec adcgfeb | bgdfc ec dfgae gefcd
...
PAT_8 = A + B + C + D + E + F + G
PAT_9 = A + B + C + D + F + G
ALL_PATS = {PAT_0: 0, PAT_1: 1, PAT_2: 2, PAT_3: 3, PAT_4: 4, PAT_5: 5, PAT_6: 6, PAT_7: 7, PAT_8: 8, PAT_9: 9}
...
def decode_simple(text):
    ...
    for line in text.split('\n'):
        _, screen = line.split('|')
        total_unique += len([val for val in screen.strip().split(' ') if len(val) in (2, 3, 4, 7)])
    return total_unique
...
def decode_full(text):
    ...
    total = 0
    for line in text.split('\n'):
        inputs, screen = line.split('|')
        scorer = decodeit(inputs)
        num_as_text = ''
        for num in screen.strip().split(' '):
            num_as_text += str(ALL_PATS[scoreit(scorer, num)])
        total += int(num_as_text)
    return total


def decodeit(inputs):
    base_scorer = {'a': A, 'b': B, 'c': C, 'd': D, 'e': E, 'f': F, 'g': G}
    for perm in permutations(base_scorer.values(), 7):
        new_scorer = dict(zip(base_scorer.keys(), perm))
        correct = True
        for word in inputs.strip().split(' '):
            word_score = scoreit(new_scorer, word)
            if word_score not in ALL_PATS:
                correct = False
        if correct:
            return new_scorer
    raise ValueError("Couldn't crack???")


def scoreit(scorer, word):
    return sum(scorer[letter] for letter in word)


##########################
if __name__ == '__main__':
    assert decode_simple(TEST2) == 26
    assert decode_simple(INPUT) == 239
    assert decode_full(TEST) == 5353
    assert decode_full(TEST2) == 61229
    assert decode_full(INPUT) == 946346
