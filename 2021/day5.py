...
"""Solutions from MaximVT (one diagram, add dots for each vector)
...
def main():
    diagram = {}
    with open("venture/venture.txt") as f:
        for line in f:
...
Solutions from ScottieX (Point and Line as clasees, Line knows what kind of line it is):
...
class Vector:
    def __init__(self, start: str, end: str, diag=False):
        self.start_x, self.start_y = [int(coord) for coord in start.split(',')]
        self.end_x, self.end_y = [int(coord) for coord in end.split(',')]
        self.diag = diag

    def cash_out_coords(self):
        if self.start_x == self.end_x:  # Vertical
            y_range = (self.end_y, self.start_y+1) if self.start_y > self.end_y else (self.start_y, self.end_y+1)
            return [(self.start_x, y) for y in range(*y_range)]
        elif self.start_y == self.end_y:  # Horizontal
            x_range = (self.end_x, self.start_x+1) if self.start_x > self.end_x else (self.start_x, self.end_x+1)
            return [(x, self.start_y) for x in range(*x_range)]
        elif self.diag:  # Diagonal
            length = abs(self.start_x - self.end_x)  # Would be the same for Y
            hor_orient = 1 if self.start_x - self.end_x < 0 else -1
            vert_orient = 1 if self.start_y - self.end_y < 0 else -1
            coords = []
            for i in range(length+1):
                coords.append((self.start_x + i*hor_orient, self.start_y + i*vert_orient))
            return coords
        return []


def main(coords_raw, diag=False):
    all_coords = []
    for coord_pair_raw in coords_raw.split('\n'):
        start, end = re.match(r'(\d+,\d+) -> (\d+,\d+)', coord_pair_raw).groups()
        vector = Vector(start, end, diag)
        all_coords.extend(vector.cash_out_coords())
    count = Counter(all_coords)
...
if __name__ == '__main__':
    # Without diagonals:
    assert main(TEST) == 5
    assert main(INPUT) == 8111

    # With diagonals:
    assert main(TEST, diag=True) == 12
    assert main(INPUT, diag=True) == 22088
